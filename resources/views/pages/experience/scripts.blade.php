...
<script>
    ...
    $(document).ready(function () {
        ...
        $("#bookexp").click(function(e){
            ...
            if( !validateParticipantsForms() ){
                ...
                swal("",'Kindly complete the booking forms.', "error");
            ...
            $.post("{{url('api/checkout_experience')}}",formData )
                .done(function (res) {
                    ...
                    if (!res) {
                        ...
                        swal("",'Failed to complete booking.', "error");
                        return false;
                    }
                    if (res.status == '400') {
                    ...
                    if (res.status == 'fail') {
                        ...
                        swal("Booking Error",'Sorry! Booking failed..', "error");
                    ...
                    if (res.status == '200') {
                        ...
                        swal("Booking ",res.message, "error");
    ...
    function nextForm(e) {
        ...
        if(participants_count > seats_available){
            ...
            swal("Booking Error",seats_available+ " Maximum seats available.", "error");
            return false;
        }
    ...
    function buildPriceOptions(data) {
        ...
        $.each(data, function (index, item) {
            var price_option = "<div class='price_option_item' data-price='"+item.price_raw+"' data-type='"+item.type+"' data-signature='"+item.signature+"'>"
            price_option += "<div>"+item.type_label+"</div>";
            price_option += "<div>"+item.price_processed+ ' '+item.currency+"</div>";
            price_option += "<div><input type='number' min='1' value='1' class='form-control' placeholder='Qty' style='width: 100px;'></div>";
